;;; lyl/ai/autoload/chatgpt-shell.el -*- lexical-binding: t; -*-

(defvar lyl-ai-chatgpt-popup-display-config
  '(("^\\*chatgpt\\* "
     (+popup-buffer)
     (actions)
     (side . top)
...
;;;###autoload
(defun lyl-ai-get-terminal-buffer-content (&optional max-chars)
...
(defun lyl-ai-send-to-chatgpt (query &optional context)
...
If CONTEXT is provided, it will be formatted as a code block before the query."
  ...
  (let* ((chatgpt-shell-prompt-query-response-style 'other-buffer)
         (display-buffer-alist lyl-ai-chatgpt-popup-display-config)
...
;;;###autoload
(defun lyl-send-to-chatgpt-if-comma-a (f &rest args)
  ...
  (let ((input (comint-get-old-input-default)))
    (if (string-prefix-p "," input)
        (let ((query (substring input 1))
              (buffer-content (lyl-ai-get-terminal-buffer-content)))
          (lyl-ai-send-to-chatgpt query buffer-content))
      (apply f args))))

;;;###autoload
(defun lyl-eshell-send-to-chatgpt-if-comma ()
...
;;;###autoload
(defun lyl-ai-toggle-shell-buffer (get-buffer-fn &optional create-fn)
...
;;;###autoload
(defun lyl-ai-toggle-chatgpt-shell ()
  "Toggle the visibility of the ChatGPT shell buffer."
  (interactive)
  (require 'chatgpt-shell)
  (lyl-ai-toggle-shell-buffer
...
;;;###autoload
(defun lyl-ai-toggle-dall-e-shell ()
...
;;;###autoload
(defun lyl-ai-chatgpt-quit-or-delete-char (arg)
...
;;;###autoload
(defun lyl-ai-open-chatgpt-workspace ()
  "Open ChatGPT in a dedicated workspace."
  (interactive)
  (+workspace-switch lyl-ai-chatgpt-shell-workspace-name t)
  (chatgpt-shell nil)
  (persp-add-buffer (current-buffer)))

;;;###autoload
(defun lyl-ai-open-dall-e-workspace ()
  "Open DALL-E in a dedicated workspace."
  (interactive)
  (+workspace-switch lyl-ai-dall-e-shell-workspace-name t)
  (dall-e-shell nil)
  (persp-add-buffer (current-buffer)))
